...
const ListPage = (() => {
  ...
  // p001: 100ml당, p004: 1L당, 그 외: 500ml당
  const formatPerUnit = (p, applied) => {
    const rate = applied ? (1 - COUPON_RATE) : 1;

    if (p.id === 'p001') {
      if (!p.pricePer100ml) return null;
      return `100ml당 ${Math.floor(p.pricePer100ml * rate).toLocaleString()}원`;
    } else if (p.id === 'p004') {
      if (!p.pricePer1L) return null;
      return `1L당 ${Math.floor(p.pricePer1L * rate).toLocaleString()}원`;
    } else {
      if (!p.pricePer500ml) return null;
      return `500ml당 ${Math.floor(p.pricePer500ml * rate).toLocaleString()}원`;
    }
  };
